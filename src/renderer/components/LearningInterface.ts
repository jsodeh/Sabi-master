...
import { LearningRequest, UserPreferences } from '../../types/learning';
import { InputType } from '../../types/common';
import { ExtendedUserPreferences } from '../../types/user';

export interface LearningInterfaceState {
...
export class LearningInterface {
  private state: LearningInterfaceState;
  ...
  public onInputTypeChange(callback: (type: InputType) => void): void {
  ...
  public updateState(updates: Partial<LearningInterfaceState>): void {
  ...
  private renderSessionControls(): string {
    if (!this.state.isSessionActive) {
      return `
  ...
  private updateInputTypeButtons(): void {
  ...
  private handleDocumentClick(e: Event): void {
    ...
    if (target.classList.contains('input-type-btn') || target.closest('.input-type-btn')) {
      const inputBtn = target.closest('.input-type-btn') as HTMLElement;
      const type = inputBtn.dataset.type as InputType;
      if (type) {
  ...
  private handleDocumentInput(e: Event): void {
  ...
  private handleInputTypeChange(type: InputType): void {
  ...
  private handleImageInput(file: File): void {
  ...
  private handleEditTranscription(): void {
    ...
    if (transcriptionText) {
      ...
      const editableArea = document.getElementById('editableTranscription') as HTMLTextAreaElement;
  ...
  private async handleVoiceInputEnhanced(): Promise<void> {
    if (this.state.isProcessing) {
      // Stop recording
      this.updateState({ isProcessing: false });
      return;
  ...
  public showInterface(): void {
    ...
    if (learningInterface) learningInterface.classList.remove('hidden');
  }
  ...
  public hideInterface(): void {
    ...
    if (learningInterface) learningInterface.classList.add('hidden');
  }

  public setPreferences(preferences: ExtendedUserPreferences): void {
    // Apply theme
